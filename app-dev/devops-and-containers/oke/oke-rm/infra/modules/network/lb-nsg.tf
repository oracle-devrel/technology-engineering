...
# OCI Native Ingress + Pods as Backends support
resource "oci_core_network_security_group_security_rule" "oke_lb_nsg_rule_pods_egress_tcp" {
  direction                 = "EGRESS"
  network_security_group_id = oci_core_network_security_group.oke_lb_nsg.id
  protocol                  = local.tcp_protocol
  destination_type = "NETWORK_SECURITY_GROUP"
  ...
  stateless = true
  description = "LB to pods, OCI Native Ingress and Pods as Backends"
  count = local.is_npn ? 1 : 0
}

resource "oci_core_network_security_group_security_rule" "oke_lb_nsg_rule_pods_egress_tcp_stateless_ingress" {
  direction                 = "INGRESS"
  network_security_group_id = oci_core_network_security_group.oke_lb_nsg.id
  protocol                  = local.tcp_protocol
  source_type = "NETWORK_SECURITY_GROUP"
...
resource "oci_core_network_security_group_security_rule" "oke_lb_nsg_rule_pods_egress_udp" {
  direction                 = "EGRESS"
  network_security_group_id = oci_core_network_security_group.oke_lb_nsg.id
  protocol                  = local.udp_protocol
  destination_type = "NETWORK_SECURITY_GROUP"
...
resource "oci_core_network_security_group_security_rule" "oke_lb_nsg_rule_pods_egress_udp_stateless_ingress" {
  direction                 = "INGRESS"
  network_security_group_id = oci_core_network_security_group.oke_lb_nsg.id
  protocol                  = local.udp_protocol
  source_type = "NETWORK_SECURITY_GROUP"
