#!/bin/bash
# ****************************************************************************************
# Script: oic3_updateConnectionFromFile_integration.sh
#
# This script is implementing comand to geupdatet connection details and test the connection - payload is expected stored in the file where the payload properties can be modified and adjusted according environment. The same payload can be used to import/update the connection the OIC environment as part of CICD Implementation for OCI/OIC 
#
# Oracle 
...
# Last updated comments:The script is importing/updating connection according payload in the file - adaptation based on https://docs.oracle.com/en/cloud/paas/application-integration/rest-api/rest-endpoints.html 
#                        
...
import_connection_api=$(curl -X POST -H "Authorization: Bearer $access_token"  -H "X-HTTP-Method-Override: PATCH" -H "Content-Type: application/json" -d @POB_OICTOOPA_INVOKE_CONN.json https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/connections/POB_OICTOOPA_INVOKE?integrationInstance=teamoic3-frrnyzlwrqhn-fr)


# Error handling
status=$(echo "$import_integration_api" | jq -r '.status')
echo "Import connection status: $status"

if [[ $status == *"409"* ]]; then
  update_existing_integration=$(curl -X PUT -H "Authorization: Bearer $access_token" -H "Accept:application/json" -F file=@POB_OICTOOPA_INVOKE_CONN.json -F type=application/octet-stream https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/integrations/archive?integrationInstance=teamoic3-frrnyzlwrqhn-fr)
