...
#
# This script is implementing comand to start a schedule for the project integration - OIC environment as part of CICD Implementation for OCI/OIC or example of trigger integration on system event
#
# Oracle 
...
# Last updated comments:The script is starting a schedule for the project integration - adaptation based on https://docs.oracle.com/en/cloud/paas/application-integration/rest-api/rest-endpoints.html 
#                        
...
echo '{"parameters":[{"name":"schedule_parameter_1","value":"1234"}],"runAsUserName":"peter.obert@oracle.com"}' > ./startschedule.json
cat ./startschedule.json

params='async=true&integrationInstance=<<your_oic_instance_name>>'

StartSchedule_Integration_api=$(curl -X POST -H "Authorization: Bearer $access_token" -H "Content-Type: application/json" -d @startschedule.json https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/projects/<<my_integrationproject_id>>/integrations/<<my_integration_id>>%7C<<version>>/schedule/start?integrationInstance=<<your_oic_instance_name>>)



# Error handling
echo "Start Schedule for Integration response: $StartSchedule_Integration_api"
# Error handling
http_links=$(echo "$StartSchedule_Integration_api" | jq -r '.links')
http_status=$(echo "$StartSchedule_Integration_api" | jq -r '.Schedule')

# Check if the StartSchedule_Integration_api response is with no content
if [[ -n "$http_status" || -n "$http_links" ]] 
then
    echo "Integration schedule start succeeded/or schedule is active with with reply: $StartSchedule_Integration_api" >&2
else 
    echo "Error: Integration schedule start failed with Response: $StartSchedule_Integration_api" >&2
    exit 1
fi
exit 0