#!/bin/bash
# ****************************************************************************************
# Script: oic3_updateConnectionFromFile_project.sh
#
# This script is implementing comand to get updated connection details and test the connection project - payload is expected stored in the file where the payload properties can be modified and adjusted according environment. The same payload can be used to import/update the connection the OIC environment as part of CICD Implementation for OCI/OIC 
#
# Oracle 
...
# Last updated comments:The script is importing/updating project connection according payload in the file - adaptation based on https://docs.oracle.com/en/cloud/paas/application-integration/rest-api/rest-endpoints.html 
#                        
...
import_connection_api=$(curl -X POST -H "Authorization: Bearer $access_token"  -H "X-HTTP-Method-Override: PATCH" -H "Content-Type: application/json" -d @POB_RESTWITH_TO_DB_ATP_PE_CONN.json https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/projects/POB_RESTWITH_TO_DB/connections/POB_ATP_DEMO_PE?integrationInstance=teamoic3-frrnyzlwrqhn-fr)


# Error handling
status=$(echo "$import_connection_api" | jq -r '.status')
echo "Import connection status: $status"

if [[ $status == *"409"* ]]; then
  update_existing_connintegration=$(curl -X PUT -H "Authorization: Bearer $access_token" -H "Accept:application/json" -F file=@POB_RESTWITH_TO_DB_ATP_PE_CONN.json -F type=application/octet-stream https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/projects/POB_RESTWITH_TO_DB/connections/POB_ATP_DEMO_PE?integrationInstance=teamoic3-frrnyzlwrqhn-fr)
  
  status=$(echo "$update_existing_connintegration" | jq -r '.status')
