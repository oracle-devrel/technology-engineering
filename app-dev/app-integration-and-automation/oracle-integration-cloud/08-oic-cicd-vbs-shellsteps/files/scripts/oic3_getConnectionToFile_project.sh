...
export_connectionSetUp_api=$(curl -X GET -H "Authorization: Bearer $access_token" -o '<<my_connection_id>>.json' https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/projects/<<my_project_id>>/connections/<<my_connection_id>>?integrationInstance=<<your_oic_instance_name>>)


# Send the GET request and capture the response headers in a separate file
response_headers=$(mktemp)
curl -X GET -o 'TESTNESUITE_CONN.json' -H "Authorization: Bearer $access_token" -D "$response_headers" https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/projects/<<my_project_id>>/connections/<<my_connection_id>>?integrationInstance=<<your_oic_instance_name>>
