#!/bin/bash
# ****************************************************************************************
# Script: oic3_getConnectionToFile_project.sh
#
# This script is implementing comand to get project connection details and store it to the file where the payload properties can be modified and adjusted according environment. The same payload can be used to import/update the connection the OIC environment as part of CICD Implementation for OCI/OIC 
#
# Oracle 
...
# Last updated comments:The script is exporting project connection payload to file - adaptation based on https://docs.oracle.com/en/cloud/paas/application-integration/rest-api/rest-endpoints.html 
#                        
...
export_connectionSetUp_api=$(curl -X GET -H "Authorization: Bearer $access_token" -o 'POB_RESTWITH_TO_DB_ATP_PE_CONN.json' https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/projects/POB_RESTWITH_TO_DB/connections/POB_ATP_DEMO_PE?integrationInstance=teamoic3-frrnyzlwrqhn-fr)


# Send the GET request and capture the response headers in a separate file
response_headers=$(mktemp)
curl -X GET -o 'TESTNESUITE_CONN.json' -H "Authorization: Bearer $access_token" -D "$response_headers" https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/projects/POB_RESTWITH_TO_DB/connections/POB_ATP_DEMO_PE?integrationInstance=teamoic3-frrnyzlwrqhn-fr
