...
import_project_api=$(curl -X POST -H "Authorization: Bearer $access_token" -H "Accept:application/json" -F file=@ORCL-R-ERP_MSEMAIL_ORDERFULFILL-01.00.0002.car -F type=application/octet-stream https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/projects/archive?integrationInstance=teamoic3-frrnyzlwrqhn-fr)
...
if [[ $status == *"409"* ]]; then
  update_existing_project=$(curl -X PUT -H "Authorization: Bearer $access_token" -H "Accept:application/json" -F file=@ORCL-R-ERP_MSEMAIL_ORDERFULFILL-01.00.0002.car -F type=application/octet-stream https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/projects/archive?integrationInstance=teamoic3-frrnyzlwrqhn-fr)
