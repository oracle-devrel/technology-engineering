...
Export a OCI Vault Key and eiether save it to file or copy the key to a different vaul
...
Usage:
  ...
  Any comandline parameters overwrites json config file values

  The script is run in two modi, eiter key export only or backup to a 2. vault.

  Parmeters/Arguments
  --configfile filename
    File with configuration parameters if a json configuration fle is used for one or more parameters
...
#
# Commandline parameters or configfile parameters and defaults
# Parammeter name, default value, boolean true/false,"Help text"
...
config_parameters = [ \
    ...
    ['exportonly',None,True,"only print info about key, don't backup"], \
    ['outputfile',None,False,'File for the exported key']
    ]

#########################################################################
# generate_signer_from_config
# Generates a OCI signer based on OCI config. Not uses in the current version
# but added for later migration to a function
#########################################################################
...
def parse_cmd_line():
    ...
    #
    # Loop throug and sett all commandline parameters
    #
...
# Any commandline settings will overwirte any seertings inherited from the 
...
#########################################################################
# verify_config
# Verifies if all mandatory parameters are set
# Take the two modi emportkey orc opykey into account
#########################################################################
def verify_config(config):
    """ verify_config, verifies is all mandatory configuration parameters is set """
    #
    ...
    #
    # Explisitt check for "source_keyname" or"source_key_ocid"
    # First set None as default if not set
    #
    ...
    if not config['exportonly']:
        ...
        if config['wrapping_pubkey_file'] is None:
...
# Generates sha1 fingerprint of the (encrypted) key
#########################################################################
def print_key_fingerprint(key): 
    ...
    hasher = hashlib.sha1()
    print(key)
    hasher.update(key.key_material.encode('utf-8'))
    print("######## Fingerprint ################")
...
# The iteration over key versions is not implemented
# returns the kms_key object if found, else False
#########################################################################
def get_kms_key(kms_client,compartment_ocid,kms_keyname,kms_keyocid,wrapping_key,wrapping_algorithm):
    """ get_kms_key """
    ...
    ###################################################
    #### Iterate over all keys in vaultsearching for the key
    ################################################### 
        ...
        if (kms_keyocid is not None and kms_keyocid == kms_key.id) or (kms_keyname is not None and kms_keyname == kms_key.display_name):
            if kms_key.protection_mode == 'SOFTWARE':
                ...
                export_key_details = oci.key_management.models.ExportKeyDetails(
                    ...
                    key_version_id=config['source_key_version']
                )
                ###################################################
...
# return when successfull, return the OCI API resonse object
#        False, Import failed
#########################################################################
...
def main():
    ...
    if config['exportonly']:
        ...
        # Extract the key
        key_data=get_kms_key(source_kms_client,config['source_compartment'],config['source_keyname'],config['source_key_ocid'],pub_key,config['wrapping_algorithm'])
        if keyd_data is not False
        ...
        #
        wrapping_key_raw = target_kms_client['management_client'].get_wrapping_key().data.public_key.replace("\n", "")
        # Extract the key
        key_data=get_kms_key(source_kms_client,config['source_compartment'],config['source_keyname'],config['source_key_ocid'],wrapping_key,config['wrapping_algorithm'])
        if key_data !=  False:
