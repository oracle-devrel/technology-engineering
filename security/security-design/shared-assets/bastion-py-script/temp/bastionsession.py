...
for PORT type SSH session, start a ssh tunnel, for managed session, connect to the target
 ...
 --exec        executes the ssh command and estabishes the ssh connection
...
__version__ = "0.8 Alpha 27.02.24"
__author__ = "Inge Os"
...
# Constants
# Dict indexes
OCI_CONFIGURATIONS="ociconfigurations"
...
def get_command(bastion_session, session_config):
    ...
    #
    # The bastion service might add  anote to the end of the command. Supress the note
    #
    if session_cmd.find(" # ") > -1:
        session_cmd = session_cmd.split(" # ")[0]

    #
    # Select session type, eithe rport forwarding or managed (through the agent)
    #
...
def process_bastion_config(config, session_name):
    ...
    if not (OCI_CONFIGURATIONS in config.keys()):
    ...
    oci_config = get_validated_config_entry(
        config[OCI_CONFIGURATIONS],
        "configName",
        session_config[OCI_CONFIG],
        mandatory_oci_keys,
    ...
    if not valdate_config(oci_config, OCI_CONFIG, oci_int_list):
        raise MissingConfigError("OCIconfig validation failed")
    #
    #  printonly must be TRUE or Y if session type= managed

    return {"sessionConfig": session_config, OCI_CONFIG: oci_config}
...
#  if exec_ssh is true,  A new session is created when the past exires
