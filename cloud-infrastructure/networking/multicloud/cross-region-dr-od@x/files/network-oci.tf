...
resource "oci_core_route_table" "hub_vcn_primary_transit_drg_rt" {
    ...
    route_rules {
        ...
        destination = var.vcn_primary_client_subnet
...
resource "oci_core_route_table" "hub_vcn_primary_transit_lpg_rt" {
    ...
    route_rules {
        ...
        destination = var.vcn_standby_client_subnet
        destination_type = "CIDR_BLOCK"
    }
}
...
# ------ Create VCN Primary and import configuration
resource "oci_core_vcn" "vcn_primary" {
  provider = oci.region-primary
  compartment_id = var.compartment_ocid
}

# ------ Create Route in VCN Primary default RT to Standby Region
resource "oci_core_default_route_table" "vcn_primary_update_default_rt" {
  provider = oci.region-primary
  compartment_id = var.compartment_ocid
  manage_default_resource_id = oci_core_vcn.vcn_primary.default_route_table_id
  route_rules {
    destination       = var.vcn_standby_client_subnet
    destination_type  = "CIDR_BLOCK"
    network_entity_id = oci_core_local_peering_gateway.primary_local_peering_gateway.id
  }
}
...
# ------ Create VCN Standby and import configuration
resource "oci_core_vcn" "vcn_standby" {
  provider = oci.region-standby
  compartment_id = var.compartment_ocid
}

# ------ Create Route in VCN Standby default RT to Primary Region
resource "oci_core_default_route_table" "vcn_standby_update_default_rt" {
  provider = oci.region-standby
  compartment_id = var.compartment_ocid
  manage_default_resource_id = oci_core_vcn.vcn_standby.default_route_table_id
  route_rules {
    destination       = var.vcn_primary_client_subnet
    destination_type  = "CIDR_BLOCK"
    network_entity_id = oci_core_local_peering_gateway.standby_local_peering_gateway.id
  }
}
