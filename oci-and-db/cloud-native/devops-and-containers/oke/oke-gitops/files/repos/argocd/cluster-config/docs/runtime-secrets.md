...
```bash
kubectl -n argocd get secret oci-devops-git-credentials
kubectl -n argocd get secret ocir-oci-repo
kubectl -n argocd get secret ocirsecret
...
5. Verify all Argo CD Applications remain `Synced` and `Healthy`.
...
[Return to first-time bootstrap](../README.md#2-install-argo-cd)
