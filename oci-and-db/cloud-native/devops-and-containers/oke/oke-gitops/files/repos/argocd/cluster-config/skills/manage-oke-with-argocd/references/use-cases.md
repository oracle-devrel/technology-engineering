...
| Add YAML rendered into `kube-system` | `cluster-config` | `platform/cluster-resources/<group>/` plus root `kustomization.yml`; never `platform/applications/` |
...
| Add spoke YAML rendered into `kube-system` | `fleet-config` | The selected profile's `cluster-resources/`; never an application folder |
