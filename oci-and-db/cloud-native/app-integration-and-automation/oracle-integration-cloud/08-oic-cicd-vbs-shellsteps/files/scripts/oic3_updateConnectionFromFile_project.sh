...
import_connection_api=$(curl -X POST -H "Authorization: Bearer $access_token"  -H "X-HTTP-Method-Override: PATCH" -H "Content-Type: application/json" -d @<<my_connection_id>>.json https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/projects/<<my_project_id>>/connections/<<my_connection_id>>?integrationInstance=<<your_oic_instance_name>>)
