#!/bin/bash
# ****************************************************************************************
# Script: oic3_import_project.sh
#
# This script is implementing comand to import project from .car file which can be used in the OIC environments as part of CICD Implementation for OCI/OIC 
#
# Oracle 
...
# Last updated comments:The script is importing project from file - adaptation based on https://docs.oracle.com/en/cloud/paas/application-integration/rest-api/rest-endpoints.html 
#                        
...
import_project_api=$(curl -X POST -H "Authorization: Bearer $access_token" -H "Accept:application/json" -F file=@<<my_integrationproject_id>>.car -F type=application/octet-stream https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/projects/archive?integrationInstance=<<your_oic_instance_name>>)


# Error handling
status=$(echo "$import_project_api" | jq -r '.status')
echo "Import project status: $status"

if [[ $status == *"409"* ]]; then
  update_existing_project=$(curl -X PUT -H "Authorization: Bearer $access_token" -H "Accept:application/json" -F file=@<<my_integration_id>>.car -F type=application/octet-stream https://design.integration.eu-frankfurt-1.ocp.oraclecloud.com/ic/api/integration/v1/projects/archive?integrationInstance=<<your_oic_instance_name>>)
  
  status=$(echo "$update_existing_project" | jq -r '.status')
  echo "Replace project status: $status"
fi
