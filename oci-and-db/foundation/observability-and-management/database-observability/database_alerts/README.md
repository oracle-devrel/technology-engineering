...
| AWR ingestion delay | Operations Insights reporting and OCI Monitoring | `AwrIngestionLag` ≥ `awr_ingestion_lag_warning_seconds` | `database-alerts` | Warning | Created conditionally when a selected Database Insight is `ENABLED` and `enable_recommended_alarms = true` |
