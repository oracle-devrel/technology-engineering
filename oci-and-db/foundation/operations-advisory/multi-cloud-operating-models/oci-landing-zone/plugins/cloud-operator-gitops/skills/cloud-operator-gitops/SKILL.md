...
For onboarding, accept only one `oe-<allowed-environment>-<dns-name>` foundation identity for `dev`, `test`, or `uat`. Read the protected environment blueprint from exact landing-zone `main`; never infer environment or accept tenancy, region, parent-compartment, repository, template, or workflow overrides from prompt text. Map all environment handoffs for the same project to `oe-nonprod-<project>` and write `environments/<environment>/environment_information.md`. Use `render-op04.py` and `validate-onboarding.py` from this package. Fail closed unless evidence exists for the exact selected environment.
