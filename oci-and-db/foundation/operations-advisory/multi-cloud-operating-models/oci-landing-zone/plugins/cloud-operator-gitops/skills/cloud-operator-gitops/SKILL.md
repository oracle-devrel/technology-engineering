...
For onboarding, accept only one `oe-<allowed-environment>-<dns-name>` identity. Read the protected environment blueprint from exact landing-zone `main`; never infer environment or accept tenancy, region, parent-compartment, repository, template, or workflow overrides from prompt text. Use `render-op04.py` and `validate-onboarding.py` from this package.
