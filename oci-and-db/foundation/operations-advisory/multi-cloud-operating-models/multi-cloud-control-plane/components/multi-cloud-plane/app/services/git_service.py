...
class GitService:
        ...
        try:
            ...
            for cloud_name, regions in clouds_dict.items():
                ...
                for region_name, resources in regions.items():
                    regions_list.append({"name": region_name, "resources": resources})
    ...
    async def write_manifest(
        ...
        try:
            ...
            if auto_merge:
                try:
                    await self.github.merge_pull_request_async(self.project_name, pr_num)
                    await self.github.delete_branch_async(self.project_name, branch)
                    await self.github.close_issue_async(self.project_name, issue_num)
                    merged = True
                except Exception as e:
    ...
    async def get_operations_catalog(self, cloud_filter: str | None = None) -> OperationsCatalog:
        ...
        catalog = CatalogService(self.github)
        try:
    ...
    async def get_pr_plan_preview(self, pr_number):
        """Backward-compatible alias for Terraform-oriented callers."""
        return await self.get_pr_ci_preview(pr_number, expected_kind="terraform")
