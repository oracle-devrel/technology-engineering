...
from glom import Coalesce, SKIP, T, glom
...
class DashboardService:
    ...
    async def get_resource_inventory(self, strict: bool = False):
        ...
        try:
            ...
            # Pass 1: collect tasks (cloud_name, region_name, relative_path, res)
            ...
            for cloud in structure.get("clouds", []):
                ...
                for region in cloud.get("regions", []):
                    ...
                    for res in region.get("resources", []):
                        ...
                        tasks.append((cloud_name, region_name, relative_path, res))

            # Pass 2: parallel manifest reads
            manifests = await asyncio.gather(
                *[
                    self.git.read_manifest(c, r, p, strict=strict)
                    for (c, r, p, _) in tasks
                ],
                return_exceptions=not strict,
            )

            # Pass 3: extract resources, preserving order
            for (cloud_name, region_name, _relative_path, res), manifest in zip(tasks, manifests):
                if isinstance(manifest, Exception):
                    continue
                extracted = self._extract_resources(
                    manifest, cloud_name, region_name, res
                )
    ...
    def _extract_resources(self, manifest, cloud, region, file_info):
        ...
        # Gather candidate sections: root manifest + *_configuration wrappers.
        sections = [manifest]
        sections.extend(
            glom(
                manifest,
                (
                    ...
                    [
                        lambda kv: kv[1]
                        if isinstance(kv[1], dict) and kv[0].endswith("_configuration")
                        else SKIP
                    ],
                ),
                default=[],
