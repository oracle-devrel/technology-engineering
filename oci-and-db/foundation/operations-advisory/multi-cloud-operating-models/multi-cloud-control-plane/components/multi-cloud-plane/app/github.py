...
class GitHubClient:
    ...
    async def merge_pull_request_async(self, repo_name: str, pr_number: int):
        """Merge a pull request."""
        self._ensure_writes_enabled()
        result = await self._request(
            self._github.rest.pulls.async_merge,
            self.org,
            repo_name,
            pr_number,
            data={},
        )
        await self.invalidate_repo_caches(
            repo_name,
            scopes={"pulls", "tree", "commits", "content", "runs", "commit_pr"},
        )
        return result

    async def delete_branch_async(self, repo_name: str, branch_name: str):
        """Delete a branch."""
        self._ensure_writes_enabled()
        await self._request(
            self._github.rest.git.async_delete_ref,
            self.org,
            repo_name,
            f"heads/{branch_name}",
        )

    async def close_issue_async(self, repo_name: str, issue_number: int):
        """Close an issue."""
        self._ensure_writes_enabled()
        return await self._request(
            self._github.rest.issues.async_update,
            self.org,
            repo_name,
            issue_number,
            data={"state": "closed"},
