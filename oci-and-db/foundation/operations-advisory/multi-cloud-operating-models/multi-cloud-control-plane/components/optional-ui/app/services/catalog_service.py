...
class CatalogService:
    ...
    def __init__(self, github_client, repo_name: str = "gitops-templates"):
    ...
    async def _tree_paths(self) -> list[str]:
        if self.repo_name == "gitops-templates" and hasattr(self.github, "get_repo_tree_strict"):
            tree = await self.github.get_repo_tree_strict(self.repo_name)
        else:
            tree = await self.github.get_repo_tree(self.repo_name)
        return [
            item["path"]
    ...
    async def load_json_template(self, path: str) -> JsonTemplate | None:
        content = await self.github.get_file_content(self.repo_name, path)
        if not content:
            return None
        try:
