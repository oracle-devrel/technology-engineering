...
These JSON Schema documents describe the OCI, Azure, and GCP Day 1 manifest
shapes published by this catalog. They are documentation artifacts only: no
workflow, runtime validator, or Terraform execution reads them.

`oci/adb.schema.json`, `oci/compute.schema.json`, and `oci/nsg.schema.json`,
plus the Azure and GCP ADB and Compute schemas, mirror the governed
