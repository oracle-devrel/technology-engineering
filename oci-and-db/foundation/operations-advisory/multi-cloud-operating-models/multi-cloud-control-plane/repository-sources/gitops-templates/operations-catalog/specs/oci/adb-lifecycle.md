...
Copy [`adb-lifecycle.json`](../../oci/adb-lifecycle.json) to the project repository path:
...
The pull request checks the request against state. After merge, the playbook reads the current lifecycle state and changes only databases not already in the requested state, so repeating the same request is safe. The workflow waits up to 30 minutes for the requested state. A target may override that with `timeout_minutes`, or disable waiting with `wait_for_state: false`; no other field is accepted.
