...
The application reads `.env` itself. `APP_URL` must match the exact browser
origin, including its protocol and port. The signed-in user remains the
authorizing identity for project writes; an optional server token may read the
catalog but cannot bypass user-scoped project authorization.
