...
forks, workflow changes, invalid JSON, mixed environments or regions, and
cross-environment placeholders.
...
- Restrict Platform CI writes to Cloud Operations and review every change to
  its `main` branch.
- Use the reviewed OCI orchestrator commit and reviewed Azure, Google Cloud, and
  GitHub Action release tags.
...
These controls are not implemented by this release. A separately tested
hardened profile can add protected `main` branches, required reviews, restricted
runner groups, and paired GitHub Environments such as `dev` and `dev-apply`.
The apply Environment can hold approval-protected secrets and prevent
self-review where the selected plan supports it.
