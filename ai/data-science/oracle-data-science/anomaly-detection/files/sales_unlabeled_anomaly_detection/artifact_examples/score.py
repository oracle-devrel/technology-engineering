# score.py 1.0 generated by ADS 2.11.19 on 20260513_095702
import json
import os
import cloudpickle
import pandas as pd
...
def deserialize(data, input_schema_path):
    ...
    if "pandas.core.frame.DataFrame" in data_type or isinstance(json_data, str):
        return pd.read_json(StringIO(json_data), dtype=fetch_data_type_from_schema(input_schema_path)) #added StringIO
...
def predict(data, model=load_model(), input_schema_path=os.path.join(os.path.dirname(os.path.realpath(__file__)), "input_schema.json")):
    ...
    prediction = float(forecast_obj.predicted_mean.iloc[0])
    ...
    return {
        ...
        'lower_prediction_interval': lower_bound,
        'upper_prediction_interval': upper_bound
    }