#!/usr/bin/env python3
"""
Generic benchmark script for non-Cohere models (Llama, Grok, Gemini, etc.)
Uses GenericChatRequest API format.

Usage:
    python generic_benchmark.py --model-name llama-3.3 --model-id $MODEL_LLAMA_3_3 ...
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark generic models + OCI guardrails.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    # Required
    parser.add_argument("--model-name", required=True,
                        help="Model name for output files (e.g., llama-3.3, grok-3, grok-4, gemini)")
    ...
    parser.add_argument("--completions", action="store_true",
                        help="Use completions API (generate_text) instead of chat API")
...
def run_model_inference(client: GenerativeAiInferenceClient, compartment_id: str,
                         model_id: str, prompt: str) -> tuple:
    """Run model inference and return (model_text, latency_ms, status)."""
    ...
    try:
        ...
        model_text = chat_resp.data._chat_response.choices[0].message.content[0].text
...
def run_model_inference_completions(client: GenerativeAiInferenceClient, compartment_id: str,
                                    model_id: str, prompt: str) -> tuple:
    """Run model inference using completions API (generate_text) for non-chat models."""
    ...
    t0 = time.time()
    try:
        resp = retry_with_backoff(client.generate_text, generate_text_detail)
        latency_ms = int((time.time() - t0) * 1000)
        model_text = resp.data.inference_response.choices[0].text
        return model_text, latency_ms, "ok"
    except oci.exceptions.ServiceError as e:
        latency_ms = int((time.time() - t0) * 1000)
        return f"[error] {e.message}", latency_ms, f"error_{e.status}"
...
def run_prompt(client: GenerativeAiInferenceClient, compartment_id: str,
               ...
               skip_guardrails: bool, skip_model: bool,
               use_completions: bool = False) -> Dict:
    ...
    if skip_model:
        ...
        inference_fn = run_model_inference_completions if use_completions else run_model_inference
        model_text, latency_ms, chat_status = inference_fn(
            client, compartment_id, model_id, prompt
        )
...
def main():
    ...
    for set_name, prompts in prompt_sets.items():
        ...
        for idx, prompt in enumerate(prompts, 1):
            print(f"[{args.model_name}] {safe_file_name} {idx}/{len(prompts)}: {prompt[:50]}...")
            result = run_prompt(
                client, args.compartment_id, args.model_id, prompt,
                args.skip_guardrails, args.skip_model, args.completions
            )
