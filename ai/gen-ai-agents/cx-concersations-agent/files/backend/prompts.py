...
# ── Tool system prompts (used by cx_tools/genai_tools.py) ────────────────────

SENTIMENT_SYSTEM = (
    "You are an expert CX analyst. Analyze the sentiment of the customer "
    "conversation the user provides. "
    "Respond with ONLY a JSON object – no markdown – containing exactly: "
...
SUMMARY_SYSTEM = (
    "You are an expert CX analyst. Summarize the following customer service "
    "conversation in 3-5 sentences. Highlight the main issue reported, "
    "the actions taken by the agent, and the resolution (if any)."
)

INTENT_SYSTEM = (
    "You are an expert CX analyst. Identify the primary intent and any "
    "secondary intents expressed by the customer in the conversation. "
    "Respond with ONLY a JSON object – no markdown – containing exactly: "
...
TOPIC_SYSTEM_TEMPLATE = (
    "You are an expert CX analyst. Classify the customer conversation into "
    "exactly one of these topics: {topics}. "
    "Respond with ONLY a JSON object – no markdown – containing exactly: "
    '{{"topic": "<topic>", "confidence": <float 0-1>}}'
)

TOPIC_DEFAULT_TOPICS = [
    "billing", "technical support", "account management",
    "product inquiry", "complaint", "other",
]

FULL_ANALYSIS_SYSTEM = (
    "You are an expert CX analyst. Given the customer service conversation "
    "the user provides, perform ALL of the following analyses in ONE response. "
    ...
    '  "topic": "<billing|technical support|account management|product inquiry|complaint|other>",\n'
