...
export default function NewFlow() {
  ...
  const handleAddTriggerPhrase = () => {
    if (newTriggerPhrase.trim()) {
      setFlow({
        ...flow,
        triggerPhrases: [...(flow.triggerPhrases || []), newTriggerPhrase.trim()]
      });
      setNewTriggerPhrase("");
    }
  };
  ...
  const handleDeleteTriggerPhrase = (index) => {
    setFlow({
      ...flow,
      triggerPhrases: flow.triggerPhrases.filter((_, i) => i !== index)
    });
  };
  ...
  const handleAddMessage = () => {
    if ((newMessage.type === 'interactive' && newInteractiveData.options.length > 0) || 
        ...
        (newMessage.type === 'text' && newMessage.content.trim())) {
      ...
      setFlow({
        ...flow,
        messages: [...(flow.messages || []), message]
      });
  ...
  const handleDeleteMessage = (index) => {
    setFlow({
      ...flow,  
      messages: flow.messages.filter((_, i) => i !== index)
    });
  };
  ...
  const handleSaveMessage = () => {
    if (editingMessageIndex !== null) {
      const newMessages = [...flow.messages];
          ...
          {activeTab === 0 && (
            <Box>
                ...
                <FormControlLabel
                  control={
                    <Switch
                      ...
                      onChange={(e) => setFlow({ ...flow, isDisabled: !e.target.checked })}
                      size="small"
                      sx={{
                  ...
                  label={flow.isDisabled ? "Disabled" : "Active"}
                  sx={{ 
                    margin: 0, 
                    "& .MuiFormControlLabel-label": {
                      fontSize: "0.85rem",
                      color: flow.isDisabled ? "rgba(0, 0, 0, 0.4)" : "#1a1a1a"
                    }
                  }}
                />
              </Box>
              ...
              <Card sx={{ p: 3, backgroundColor: "rgba(255, 255, 255, 0.7)", backdropFilter: "blur(10px)" }}>
                ...
                <List sx={{ p: 0 }}>
                  {flow.triggerPhrases?.map((phrase, index) => (
                    <ListItem
                      key={index}
                      sx={{
                        px: 2,
          ...
          {activeTab === 1 && (
            <Box>
              <Typography variant="h6" sx={{ mb: 3, fontSize: "1.1rem", fontWeight: 400 }}>
                Messages ({flow.messages?.length || 0})
              </Typography>
              <Card sx={{ p: 3, backgroundColor: "rgba(255, 255, 255, 0.7)", backdropFilter: "blur(10px)" }}>
