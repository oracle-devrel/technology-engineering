# How To Provide Network Insulation during a Full Stack DR Drill 

## Disclaimer

What presented here is just an example of a possible solution and its implementation. It is has been tested for a specific
use-case in a specific environment and it should be tested extensively before using it in a production environment.  


## FSDR Drill Feature Overview

Full Stack Disaster Recovery (FSDR) _Drill_ feature allows to test the DR without effecting the primary system. The feature works by starting
up the workload in the standby region as a _copy_ of the primary system. To make a simple example lets assume having a three tiers workload with
...
When performing a FSDR _Drill_ one possible problem is triggering external side affects that are not easy to rollback. For example, 
...
documents and sending them to external recipient. Creating those documents might be easy to rollback but explaining to the recipients 
to just ignore them might be not. Normally, we also don't want for the drill workload in the standby to be able to reach the primary, 
for example, using a _Remote Peering Connection_ between VCN. 

## The Scenario

To showcase how to solve the problem previously describe, we consider the a simple scenario with a three tiers workload and
...
While _Cross-Region Block Storage Replication_ and _Autonomous DataGuard_ don't need for the VCNs to be connected, the _AD Replication_ 
...
rules back. To accomplish that we need to updated the autogenerates _Start Drill_ and _Stop Drill_ FSDR plans with custom steps
...
will focus on removing the rule n. 3 because we want for the _Standby AD_ to take over the services in the drill workload, that
...
- the first function will be called _remove-route_ and its JSON payload will contain the route table OCID (_rt_id_) and the destination
  CIDR block (_cidr_)
- the second functions will be called _add-route_ and its JSON payload will contain the route table OCID (_rt_id_), the destination
...
Then we have t create the policy that allows the dynamic group to manipulate the network components
...
```
we can the inspect the context, for example
...
_hello world_function from the template:
...
At this point, before actually deploying the hello function we need to login the to the OCI registry. 
...
Naturally we change the code in _func.py_ and we need to add the OCI SDK package in the _requirements.txf_ file:
...
To generate the code and validate it we used Oracle Code Assist. The _remove-route_ function is the the following:
...
Below all the steps necessary to add the custom _Plan Group_ to the autogenerated _Start Drill_. We assumes only to remove the DRG route
but we can can the function in multiple steps to remove multiple route rules and they are executed in parallel.
...
![Modified _Stop Drill_ Plan](./files/images/fsdr-setup_05.png)
