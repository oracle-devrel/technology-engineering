...
<!-- GUIDANCE -->
<!--
A section describing the key values of the solution for the customer, allowing the customer to make decisions quickly.
...
<!-- GUIDANCE -->
<!--
A clear statement of specific business value as part of the full workload scope. Try to keep it SMART: Specific, Measurable, Assignable, Realistic, and Time-Related. Agree on the SMART business value with the customer. Keep it business-focused, and speak the language of the LOB which benefits from this Workload.
...
<!-- GUIDANCE -->
<!--
Describe the Workload: What applications and environments are part of this Workload, and what are their names?
...
It is important to understand who is using the system and for what reason.
...
In addition to the above functional modules, the Hyperion application is built on the middle-tier application server (Weblogic by default) and DB tier (Oracle or MS SQL Server).
...
<!-- GUIDANCE -->
<!--
This section captures any specific regulatory and compliance requirements for the Workload. These may limit the types of technologies that can be used and may drive some architectural decisions.
...
- What are SLAs of the application?
...
<!-- GUIDANCE -->
<!--
Add subsections, if cleaner separation of Resilience and Recovery into HA, DR, and Backup & Restore is needed.
...
(1) MaxVol: maximum volume of data that can be stored in the system (can be different for different types of data, e.g. relational and file): **800-900GB** current database size (probably with a significant waste of space)
...
- Is the OS hardened, if so please share the hardening guidelines.
...
<!-- GUIDANCE -->
<!--
The Workload Future State Architecture can be described in various forms. In the easiest case just describe a Logical Architecture, possibly with a System Context Diagram.
...
Provide a high-level logical solution for the complete Workload. Indicate Oracle products as abstract groups, and not as physical detailed instances.
...
<!-- GUIDANCE -->
<!--
Refer to architecture best practices or design patterns. Explain how they are reflected in your architecture and how they improve the solution.
-->
...
The customer is responsible for setting the access rules to services and environments that require stakeholders’ integration into the tenancy to comply with all applicable regulations.

## Operations
<!-- GUIDANCE -->
<!--
In this chapter provide a high-level introduction to various operations-related topics around OCI.

The below example text represents the first asset from this catalogue PCO#01. Please consider including other assets as well.
...
<!-- GUIDANCE -->
<!--
Estimate and size the resources needed for the Workload. The information can be collected and is based upon previously gathered capacities, business user numbers, integration points, or translated existing on-premises resources. The sizing is possibly done with or even without a Physical Architecture. It is ok to make assumptions and to clearly state them!

Clarify your assumptions and your sizing.
...
<!-- GUIDANCE -->
<!--
Create a detailed physical solution architecture for implementation. It will include physical attributes such as IP ranges or compute shapes. The architecture is ready for implementation.
...
<!-- GUIDANCE -->
<!--
Emphasize specific aspects of the solution architecture. See the non-functional requirements list for inspiration and identify important non-functional requirements for your customer. Apply 'Just enough architecture', do not describe every possible non-functional requirement as an architecture view.
...
<!-- GUIDANCE -->
<!--
The purpose of this section is to document all the technical items that the customer is expected to undertake, for example:

- ingress/egress between on-premises & OCI so the customer firewall can be configured
...
<!-- GUIDANCE -->
<!--
The final 'alive' project plan gets maintained throughout the complete implementation. Do not replicate a project plan in this document, it is a separate document which is only mentioned here. Link to a shared repository, such as customer Confluence Spaces, OraSites or possible slack channels. Alternatively, project plans and updates can be shared via email.

Include Customer Responsibilities from the previous chapter.
...
<!-- GUIDANCE -->
<!--
The Deployment Build is a list of all OCI resources needed for the implementation. It serves two purposes: as a customer's documentation and as an implementer handover. The checklist below defines mandatory requirements as per the project scope for implementation. This table replaces the CD3 file for the architecture team.
...
<!-- Guidance -->
<!--
Please group the Deployment Build into phases depending on the size of the engagement. Each chapter of the Deployment Build belongs to a phase and represents an iterative implementation. Rearrange the chapters to fit the right phases. An implementation could have as many phases as needed, and if it has just one, do not group it into just a single phase.
...
Cost Tracking: Specify "Yes" if the tag is for cost tracking, else "No".
-->
...
<!-- GUIDANCE -->
<!--
All the fields are mandatory except Tags and DNS labels. If DNS Label is left empty, DNS Label will be created with VCN Name.
IGW:- Internet Gateway, mention the name of the internet gateway, enter None if not needed, if left empty it will be treated as None.
SGW:- Service Gateway, mention the name of Service gateway, enter None if not needed, if left empty it will be treated as None.
NGW:- NAT Gateway, mention the name of NAT gateway, enter None if not needed, if left empty it will be treated as None.
DRG:- Dynamic Routing Gateway, enter the name of Dynamic Routing gateway, mention None if not needed, if left empty it will be treated as None. DRG should be unique in the region. If the same DRG is to be attached to multiple VCNs please mention the same in respective VCNs.
Region:- Region under which VCNs are to be created
-->
...
onprem_destinations: Enter on-premises CIDR, separated by a comma.
ngw_destination: Enter NAT Gateway CIDR.
igw_destination: Enter Internet Gateway CIDR.
subnet_name_attach_cidr: Mention y if you want to attach AD and CIDR to the object's display name; defaults to n.
-->
<!-- EXAMPLE / TEMPLATE -->
<!--START-->
Property | Value
...
Security List Name:- Specify the security list to be attached to the subnet, if left blank security list with the name as that of the subnet will be attached,
specify None if not to attach any custom security list.
Route Table Name:- Specify the Route Table Name to be attached to the subnet, if left blank route Table with the name as that of the subnet will be attached,
specify None if no custom route table should be used.
-->
...
-->
<!-- WIP: Split IPSEC and Virtual Circuit into two columns (By JC, to be reviewed by Impl) -->
...
<!--
All the fields are mandatory except Tags.
Table Compartment: Specify the Compartment in which the route table is to be created.
...
Target: Valid Targets is the name of the DRG/IGW/SGW/LPG/Private IP/NGW
-->
...
Destination Port: Specify the egress port to allow.
...
Destination: Specify Destination CIDR.
Protocol: Specify the protocol For example TCP/HTTP/ICMP
Destination Port: Specify the ingress port to allow.
...
OS Image:- Valid Values are image names with a version without a period (.) For Example, OracleLinux79 or Windows2012 to create an instance from boot volume OCID.
...
<!--
All the fields are mandatory except tags.
Size (in GB):- Specify Block Volume size in GBs.
Availability Domain:- Valid values are AD1/AD2/AD3, make sure to specify the same AD in which instance is provisioned.
...
Backend Server:Port: Specify Backend Server name:Port. The backend server should exist in the tenancy.
...
<!--
All the fields are mandatory.
LB Name: Specify the Load Balancer name that already exists in the Load Balancer Chapter.
Backend Set Name: Specify the Backend Set name that already exists in the Load Balancer Chapter.
Hostname: Specify the hostname name that already exists in the Load Balancer Chapter.
...
DB Size: Specify DB Size in GBs.
...
<!--
All the Fields are mandatory
Display Name: Specify the display name that should exist in DBSystem Info Chapter.
Availability Domain: Valid Options are AD1/AD2/AD3, AD varies from Region to Region.
...
<!--
All the Fields are mandatory
Display Name: Specify the display name that already exists in Autonomous Database Information Chapter.
...
Please avoid describing products, and include the link to product documentation at the first occurrence of a product.
